...
**Live coding round**: when `persona === 'technical'` (not a debrief) and `hasCodingFocusArea(focusAreas)` matches (`lib/personas.ts`), the route calls `generateCodingQuestion(roleTitle, focusAreas)` (`lib/coding-question.ts`) and folds the result into the technical persona's system prompt as its first substantive question. On success it best-effort upserts `context.coding_question` into `candidateContext` (same read-modify-write shape as `handoff_log`) and includes `codingQuestion` in the response.
...
```json
{ "agent_id": "...", "create_ts": 1710000000, "state": "RUNNING", "codingQuestion": { "question": "...", "testExamples": [{ "input": "...", "output": "..." }] } }
```

`codingQuestion` is present only when the live-coding-round conditions above are met; absent otherwise (including for every non-technical persona and the debrief variant).
...
### `PATCH /api/session/[id]/code`

Body: `{ "code": string }`. `400` if `code` is missing or not a string; `404` if no `candidateContext` row exists for the session. Merges `context.coding_submission = { code, updatedAt: ISOString }` into the session's `candidateContext` and returns `{ "ok": true }`. Called by `ConversationComponent.tsx`'s debounced autosave (~1.8s after the last keystroke in the coding panel's textarea) and its `flushCodeSave` immediate-write escape hatch (coding-phase timer zero-crossing, persona switch, end of call). Read back by `POST /api/session/[id]/report` when building the feedback report's `codingExercise`.
...
- `ConversationComponentProps`: runtime dependencies for in-call component — `agoraData`, `rtmClient`, `onTokenWillExpire`, `onEndConversation(transcript: ReportTranscriptTurn[])`, `currentPersona`, `activePersonas`, `personaDurationsSeconds: Record<PersonaId, number>` (recruiter-configured per-persona time budget, drives the auto-switch countdown), `isSwitchingPersona`, `switchError`, `onSwitchPersona(next, transcriptText): Promise<boolean>`, `codingQuestion?: CodingQuestion` (the live coding question, when generated), `sessionId: string` (autosave route target).
- `AgentResponse`: `{ agent_id, create_ts, state, codingQuestion?: CodingQuestion }` — the `POST /api/invite-agent` success payload; `codingQuestion` (imported from `lib/coding-question.ts`) is present only when the technical persona's focus areas triggered the live coding round.

...
`app/api/invite-agent/route.ts` receives `priorContext` and `fromPersona` on `ClientStartRequest`. If `priorContext` is present and `GROQ_API_KEY` is configured, the route first calls `groqRespond()` ([`lib/groq.ts`](../../../../lib/groq.ts)) with an instruction to summarize the raw transcript in under 200 words, preserving concrete facts the candidate stated and noting which topics are already covered. If Groq returns a summary, that summary is used as `contextForPrompt`; otherwise (no `GROQ_API_KEY`, or the Groq call fails) `contextForPrompt` falls back to the raw `priorContext` transcript unchanged — the pre-Groq behavior. `contextForPrompt` is then passed into `buildPersonaSystemPrompt(personaId, roleTitle, focusAreas, contextSoFar, durationMinutes, isFirstActivePersona, codingQuestion)` in [`lib/personas.ts`](../../../../lib/personas.ts) — the final `codingQuestion` param is set only for the technical persona when its focus areas match `hasCodingFocusArea`; see the "Live Coding Round" pattern in [`AGENTS.md`](../../../../AGENTS.md) for the generation/eligibility/timer mechanism, which follows this same file's structural/timer-driven philosophy rather than parsing LLM speech. When `contextSoFar` is present, the built prompt appends a "# Conversation So Far" section instructing the new persona not to repeat questions already asked and not to re-introduce the panel formally. `buildPersonaGreeting(personaId, roleTitle, isHandoff, fromPersonaId, isFirstActivePersona)` also switches to a shorter hand-off greeting (e.g. *"Thanks — I'm the {label} panelist, and I'll take it from here."*) instead of the cold-open greeting used for the very first persona.

### Per-Persona Focus Areas and the First-Persona Introduction

`focusAreas` passed into `buildPersonaSystemPrompt` is no longer the flat, panel-wide `sessions.focusAreas` list — it is `session.personaFocusAreas[personaId]`, looked up from the new `personaFocusAreas: jsonb` column on `sessions` ([`lib/db/schema.ts`](../../../../lib/db/schema.ts)), keyed per `PersonaId` and set per-panelist on [`SetupScreen.tsx`](../../../../components/SetupScreen.tsx) (mirroring the existing per-persona minutes UI). The flat `sessions.focusAreas` column is kept, but only as the derived union of the per-persona lists — computed client-side in `SetupScreen.handleSubmit` and re-derived defensively in `app/api/session/route.ts` — for the consumers that still want a single flat list (`lib/report.ts`'s `focusAreaCoverage`, `candidateContext.context.role_profile.focus_areas`).

Given only its own list, each persona's prompt is now a **hard constraint**, not a soft preference: `buildPersonaSystemPrompt` instructs the persona to stay strictly inside its assigned focus areas and to steer back (one short acknowledging phrase, then redirect) if the candidate raises a topic outside that list — even one that sounds like it belongs to another panelist's lane. A persona with no focus areas assigned falls back to staying within its general lane (`persona.focus`) instead.

`isFirstActivePersona` (`session.activePersonas[0] === personaId`, computed server-side in `invite-agent/route.ts`; falls back to `personaId === 'technical'` when there's no `session_id`, e.g. the contract tests) replaces the old hardcoded "technical persona always opens with the introduction" behavior — whichever persona the recruiter's active-panel ordering puts first is the one whose greeting asks for the candidate's introduction and whose prompt skips re-asking for it.

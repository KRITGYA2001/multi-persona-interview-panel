...
async function verifyInviteAgentSuccess() {
  ...
  Agent.prototype.createSession = (function (this: Agent, sessionConfig: unknown) {
    ...
    capturedInstructions = this.instructions;
    return {
  ...
  try {
    ...
    assert(
      typeof capturedInstructions === 'string' &&
        !capturedInstructions.includes('Conversation So Far'),
      'POST /api/invite-agent should not inject a hand-off section without priorContext',
    );
  } finally {
...
async function verifyInviteAgentPersonaSwitchInjectsContext() {
  const { POST: inviteAgent } = await import('../app/api/invite-agent/route');
  const originalCreateSession = Agent.prototype.createSession;
  // Force the raw-transcript fallback path (no live Groq call) so this assertion
  // stays deterministic regardless of whether GROQ_API_KEY is set in the environment.
  const originalGroqApiKey = process.env.GROQ_API_KEY;
  delete process.env.GROQ_API_KEY;
  let capturedInstructions: string | undefined;
  let capturedAgentUid: string | undefined;

  Agent.prototype.createSession = (function (this: Agent, sessionConfig: unknown) {
    capturedAgentUid = (sessionConfig as { agentUid?: string }).agentUid;
    capturedInstructions = this.instructions;
    return {
      start: async () => 'mock-agent-id-2',
    };
  }) as unknown as typeof Agent.prototype.createSession;
  ...
  try {
    const request = new NextRequest('http://localhost:3000/api/invite-agent', {
      body: JSON.stringify({
        ...
        channel_name: 'test-channel',
        persona: 'product',
        priorContext: '[Technical]: Tell me about a system you built.\n[Candidate]: I built a queue.',
    ...
    assert(
      response.status === 200,
      'POST /api/invite-agent should return 200 for a persona switch',
    );
    assert(
      body.agent_id === 'mock-agent-id-2',
      'POST /api/invite-agent should return the new agent id on switch',
    );
    assert(
      capturedAgentUid === '123456',
      'POST /api/invite-agent should keep the same agentUid across a persona switch',
    );
    assert(
      typeof capturedInstructions === 'string' &&
        capturedInstructions.includes('Conversation So Far') &&
        capturedInstructions.includes('I built a queue.'),
      'POST /api/invite-agent should inject priorContext into the new persona instructions',
    );
  } finally {
    Agent.prototype.createSession = originalCreateSession;
...
async function verifyReportRouteValidation() {
  const { POST: postReport } = await import('../app/api/session/[id]/report/route');
  const request = new NextRequest(
    'http://localhost:3000/api/session/00000000-0000-0000-0000-000000000000/report',
    {
  ...
  assert(
    response.status === 400,
    'POST /api/session/[id]/report should reject an empty transcript',
  );
  assert(
    body.error === 'transcript is required and must be non-empty',
    'POST /api/session/[id]/report should explain the validation failure',
  );
}

async function verifyReportHeuristicFallback() {
  const { buildFeedbackReport } = await import('../lib/report');
  const originalApiKey = process.env.NEXT_LLM_API_KEY;
  const originalUrl = process.env.NEXT_LLM_URL;
  const originalGroqApiKey = process.env.GROQ_API_KEY;
  delete process.env.NEXT_LLM_API_KEY;
  ...
  try {
    const report = await buildFeedbackReport({
      ...
      transcript: [
        { persona: 'technical', speaker: 'panelist', text: 'Tell me about a system you built.' },
        {
          persona: 'technical',
          speaker: 'candidate',
          text: 'I built a distributed systems queue that processed millions of events per day with strong consistency guarantees.',
        },
      ],
    });
    ...
    assert(
      ...
      'buildFeedbackReport should fall back to the heuristic report without LLM env vars',
    );
    assert(
      report.roleTitle === 'Staff Engineer',
      'buildFeedbackReport should echo the role title',
    );
    assert(
      report.personas.length === 1 && report.personas[0].persona === 'technical',
      'buildFeedbackReport should include a section for every persona present in the transcript',
    );
    assert(
...
async function verifySessionRouteRejectsMissingRecruiterEmail() {
  ...
  assert(
    response.status === 400,
    'POST /api/session should reject a missing recruiterEmail',
  );
  assert(
    body.error === 'A valid recruiterEmail is required',
    'POST /api/session should explain the missing recruiterEmail',
  );
}
...
async function verifySessionRouteRejectsInvalidRecruiterEmail() {
  ...
  assert(
    response.status === 400,
    'POST /api/session should reject a malformed recruiterEmail',
  );
  assert(
    body.error === 'A valid recruiterEmail is required',
    'POST /api/session should explain the malformed recruiterEmail',
  );
}
